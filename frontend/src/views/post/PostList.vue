  ...
  <div class="post-list-page">
    <div class="post-list-main">
      <div class="post-list-container">
          ...
          >
            ...
            <div class="post-footer">
              ...
              <div v-if="post.tags && post.tags.length > 0" class="tags">
                <a-tag v-for="tag in post.tags" :key="tag" size="small" color="arcoblue">
                  {{ tag }}
                </a-tag>
              </div>
            </div>
        ...
        <div v-if="!loading && posts.length > 0" class="pagination-wrapper">
          <a-pagination
...
import { Card, Skeleton, Empty, Pagination, Tag, Message } from '@arco-design/web-vue';
import {
  IconEye,
  IconStar,
  IconMessage,
...
import type { PostSummaryVO } from '@/types';
...
const loadPosts = async () => {
  ...
  try {
    ...
    if (response.code === 200) {
      posts.value = response.data.rows as PostSummaryVO[];
...
watch(
  () => [pagination.value.currentPage, pagination.value.pageSize, searchQuery.value],
  () => {
    loadPosts();
  }
);
